<?php
    ...
    function oldmt_search(){
        ...
        if($tid == NULL && $godate == NULL && $backdate == NULL){
            $sql = "SELECT * FROM `old_travels` WHERE `mid` = '$mid'";
            ...
            function oldmt_ctravels(){
                $mid = $_POST['mid'];
                $sql = "SELECT `mid`, COUNT(`mid`) FROM `old_travels` WHERE `mid` = '$mid'";
                $result = filterTable($sql);
                while($row = mysqli_fetch_array($result)):
                    echo "<tr>";
        ...
        elseif($mid == NULL && $godate == NULL && $backdate == NULL){
            $sql2 = "SELECT * FROM `old_travels` WHERE `tid` = '$tid'";
            ...
            function oldmt_ctravels(){
                $tid = $_POST['tid'];
                $sql = "SELECT `tid`, COUNT(`tid`) FROM `old_travels` WHERE `tid` = '$tid'";
                $result = filterTable($sql);
                while($row = mysqli_fetch_array($result)):
                    echo "<tr>";
        ...
        elseif($mid == NULL && $tid == NULL){
            $sql3 = "SELECT * FROM `old_travels` WHERE `godate` BETWEEN '$godate' AND '$backdate'";
    ...
    /*function oldmt_smoney(){
        $sql = "SELECT `mid`, SUM(`total`) FROM `old_travels` GROUP BY `mid`";
        $result = filterTable($sql);
            while($row = mysqli_fetch_array($result)):
                echo "<tr>";
                echo "<td>".$row['mid']."</td>";
    ...
    function oldmt_avgtravels(){
        $sql = "SELECT `mid`, ROUND(AVG(`total`), 2) FROM `old_travels` GROUP BY `mid`";
        $result = filterTable($sql);
            while($row = mysqli_fetch_array($result)):
                echo "<tr>";
                echo "<td>".$row['mid']."</td>";
                echo "<td>".$row['ROUND(AVG(`total`), 2)']."</td>";
                echo "</tr>";
            endwhile;
    }*/
?>